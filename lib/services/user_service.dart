...
class UserService {
  ...
  Future<void> updateFirebaseUser(
    ...
    return authService.users.doc(user!.uid).update(data).then((value) {
      vibrantSnackBar(context: context, content: 'Location updated on database');
    }).catchError((error) {
      vibrantSnackBar(
          context: context,
  ...
  updateFavourite(
    ...
    if (isLiked) {
      ...
      vibrantSnackBar(context: context, content: 'Added to favourites');
    } else {
      authService.products.doc(productId).update({
        'favourites': FieldValue.arrayRemove([user!.uid])
      });
      vibrantSnackBar(context: context, content: 'Removed from favourites');
    }
  }
}
